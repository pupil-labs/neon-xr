...
extern "C" {
	__declspec(dllexport) float* CGetCameraPose(int npoints, float* objectPoints, float* imagePoints, float* cameraMatrix, float* distCoeffs) {
	...
	__declspec(dllexport) void CFree(void* ptr) {
		if (ptr != NULL)
		{
			free(ptr);
